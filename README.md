...
This version keeps the existing Top Speed Analyzer UI and Relay Baton Analyzer UI unchanged. The only added UI is the top-level switch between:

- トップスピード分析
- バトンパス分析

The relay analyzer is served as an isolated Vite HTML entry at `/baton.html` so that its original Tailwind-based UI is preserved.
